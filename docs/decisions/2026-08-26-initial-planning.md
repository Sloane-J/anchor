...
**Decision:** Build a dependable foreground `dev start` before daemonisation, restart policies, health checks, a TUI, plugins, or integrations.
...
- `dev stop`, `dev status`, and `dev logs` are roadmap commands, not commands to stub now.
